...
if __name__ == '__main__':
    ...
    for elevm in vm_grid_list:
        elevm.click()
        stock_list = driver.find_elements_by_xpath("//android.widget.TextView[contains(@text,'库存:')]")
        for stcok in stock_list:
            if stcok.text == "库存:0":
                pass
            else:
                logPrint("清零补货验证：FAIL!!")
        driver.find_element_by_xpath("//android.widget.TextView[@text='返回']").click()
    driver.find_element_by_xpath("//android.widget.TextView[@text='返回']").click()
    driver.find_element_by_xpath("//android.widget.TextView[@text='一键补满']").click()
    driver.find_element_by_xpath("//android.widget.Button[@text='确定']").click()
    try:
        WebDriverWait(driver, 180).until_not(EC.presence_of_element_located(loadMask_locator))
        completeFlag = True
        ...
        completeFlag = False
    if completeFlag:
        logPrint("一键补满完成：PASS")
    else:
        logPrint("一键补满完成：FAIL!!")
    ...
    if stock_list == capacity_list:
        logPrint("一键补满成功：PASS")
    else:
        logPrint("一键补满成功：FAIL!!")
